...
| `/reservations/availability/` | `reservation-availability` | none | `POST`: `guest_count`, `reservation_date` (`YYYY-MM-DD`, today or later), and `time_slot` (time-slot ID). | `200 OK` HTML fragment; `405 Method Not Allowed` for `GET`. | Return availability for HTMX to replace the form result region. This creates no customer or reservation. |
