...
The system SHALL expose a URL that lists restaurant tables.

#### Scenario: Visitor opens table list
- **WHEN** a visitor requests `/tables/`
- **THEN** the system returns a successful response containing table records ordered by capacity and table number

### Requirement: Time slot list is callable
The system SHALL expose a URL that lists reservation time slots.

#### Scenario: Visitor opens time slot list
- **WHEN** a visitor requests `/time-slots/`
- **THEN** the system returns a successful response containing time slot records ordered by start time
