...
- `FancyRestaurant/`: Django project package.
- `FancyRestaurant/settings.py`: default Django settings, currently using SQLite.
- `FancyRestaurant/urls.py`: root URL configuration with the admin route and `FancyRestaurantApp` URLs.
- `FancyRestaurant/asgi.py`: ASGI application entrypoint.
- `FancyRestaurant/wsgi.py`: WSGI application entrypoint.
- `FancyRestaurantApp/`: Django app for the reservation domain. Its Django app label remains `reservations` so the existing migrations and database tables remain compatible.
- `.agents/skills/`: project-installed AI skills for Django work, code review, review feedback, and refactoring.
- `.opencode/`: OpenCode commands and OpenSpec skills generated by `openspec init`.
- `openspec/`: OpenSpec project configuration and planning artifacts.
- `skills-lock.json`: lock metadata for installed project skills.
- `main.py`: original scaffold entrypoint; not part of the Django runtime.
- `tests/test_sample.py`: current pytest smoke test.
- `pytest.ini`: pytest configuration for `FancyRestaurant.settings`.
- `.pylintrc`: pylint configuration with the Django plugin.

`FancyRestaurantApp` is registered in `FancyRestaurant/settings.py`; its Django app label is `reservations`.
...
- OpenCode is the primary coding agent for this project.
- OpenSpec is initialized for OpenCode. Use OpenSpec planning for non-trivial changes before implementation.
- Project skills are installed under `.agents/skills`; review skill files before relying on newly added external guidance.
- For OpenSpec workflows, start with `/opsx-propose`, implement with `/opsx-apply`, and archive completed changes with `/opsx-archive`.
